...
<div id="recalculateHoursModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-60 flex items-center justify-center p-4">
    <div class="bg-white bg-opacity-90 rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all text-gray-800" style="max-height: 90vh; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
        ...
        checkShiftsBtn.addEventListener('click', () => {
            ...
            fetch('../admin/ajax_find_shifts_without_hours.php').then(res => res.json()).then(data => {
                ...
                if (data.success && data.shifts.length > 0) {
                    let html = `<h3 class="text-lg font-semibold mb-3">Знайдено <span class="text-red-500 font-bold">${data.shifts.length}</span> змін:</h3><div class="max-h-64 overflow-y-auto border rounded-lg p-3 bg-white bg-opacity-40 mb-4">`;
