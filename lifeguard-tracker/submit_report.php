...
try {
    ...
    if ($stmt_check->fetch()) {
        ...
        header('Location: index.php'); // Або на сторінку історії змін лайфгарда
        exit();
    }
...
try {
     $stmt_shift = $pdo->prepare("
        SELECT s.id, s.start_time, s.end_time, p.name as post_name, u.full_name as lifeguard_name
        FROM shifts s
    ...
    if (!$shift_info) {
        set_flash_message('помилка', 'Зміну не знайдено, або вона не завершена, або не належить вам.');
        header('Location: index.php');
        exit();
    }
} catch (PDOException $e) {
    // error_log("Report Shift Info DB Error: " . $e->getMessage());
    set_flash_message('помилка', 'Не вдалося завантажити інформацію про зміну.');
    header('Location: index.php');
    exit();
}
